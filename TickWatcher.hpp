...
/// <param name="TimeUnit">Time ratio, for example std::milli. Must be lesser
/// than std::ratio<1, 1></param>
/// </summary>
...
class TickWatcher {
 ...
 public:
  ...
  TickWatcher(NumberT tickrate) { setTickrate(tickrate); }
  ...
  void setTickrate(NumberT hertz) {
    ...
    m_tick_time = Duration{static_cast<NumberT>(
        tick_time_seconds * static_cast<double>(TimeUnit::den))};
  }

  /// <summary>
  /// Sets time for a tick
  /// <param name="time">Time, unit as passed in TimeUnit template
  /// argument</param>
  /// </summary>
  void setTickTime(NumberT time) { m_tick_time = Duration{time}; }
  ...
  double getTickrate() const {
    return 1. / static_cast<double>(
                    m_tick_time.count() /
                    static_cast<typename Duration::rep>(TimeUnit::den));
  }

  /// <summary>
  /// Gets tick time (in TimeUnit)
  /// </summary>
  NumberT getTickTime() const { return m_tick_time.count(); }
  ...
  /// </summary>
  void nextTick() { m_last_tick = Timer::now(); }

  /// <summary>
  /// If tick time has happened, calculates next tick time and returns true. If
  /// not, returns false.
  /// </summary>
  bool tick() {
    if (shouldTick()) {
